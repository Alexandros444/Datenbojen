...
#define PHVALUEADDR PH_EEPROM_ADDRESS    //the start address of the pH calibration parameters stored in the EEPROM
...
void DFRobot_PH::begin()
{
    ...
    DPRINTF("Neutral Voltage loaded from EEPROM: %f mV\n", this->_neutralVoltage);
    ...
    DPRINTF("Acid Voltage loaded from EEPROM: %f mV\n", this->_acidVoltage);
    ...
    if (_neutralVoltage == 0.0 || _acidVoltage == 0.0) {
        Serial.println(F("Neutral or Acid Voltage is zero, resetting EEPROM to default values."));
        resetEEPROM(); // Reset EEPROM to default values if neutral or acid voltage is zero
    }

}
    void DFRobot_PH::resetEEPROM() {
    this->_neutralVoltage = 1500.0;  // reset to default value
    this->_acidVoltage = 2032.44;     // reset to default value
    EEPROM_write(PHVALUEADDR, this->_neutralVoltage);
    EEPROM_write(PHVALUEADDR+4, this->_acidVoltage);
...
#ifdef DEBUG_PRINT
    Serial.printf("Reading PH value with voltage: %f mV, temperature: %f C\n", voltage, temperature);
    Serial.printf("Neutral Voltage: %f mV, Acid Voltage: %f mV\n", this->_neutralVoltage, this->_acidVoltage);
    Serial.printf("slope: %f, intercept: %f, calculated pH value: %f\n", slope, intercept, this->_phValue);
#endif
    return _phValue;
}
