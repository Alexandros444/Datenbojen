...
int LoRaClass::begin(long frequency)
{
  ...
  pinModePCF(LORA_IRQ_DUMB, OUTPUT);
  digitalWritePCF(LORA_IRQ_DUMB, LOW);

  // Hardware reset
  pinModePCF(LORA_BOOT0, OUTPUT);
  digitalWritePCF(LORA_BOOT0, LOW);

  pinModePCF(LORA_RESET, OUTPUT);
  digitalWritePCF(LORA_RESET, HIGH);
  delay(200);
  digitalWritePCF(LORA_RESET, LOW);
  delay(200);
  digitalWritePCF(LORA_RESET, HIGH);
  delay(50);
#endif
  ...
  // setup pins
  pinModePCF(_ss, OUTPUT);
  // set SS high
  digitalWritePCF(_ss, HIGH);

  if (_reset != -1) {
    pinModePCF(_reset, OUTPUT);

    // perform reset
    digitalWritePCF(_reset, LOW);
    delay(10);
    digitalWritePCF(_reset, HIGH);
    delay(10);
  }
  ...
  if (version != 0x12) {
    Serial.printf("LoRa: invalid version 0x%02x\n", version);
    return 0;
  }
